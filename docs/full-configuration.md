...
- **"common"**, configuration parameters shared among all the supported protocols
  ...
  - `add_protocol_metadata`, boolean. Setting this to true will attach metadata to files uploaded to S3, GCS, and Azure Blob which states which protocol was used to upload the file from the client, useful for data lineage. For example: `{"protocol": "sftp"}`. Leave empty or set to false to disable.
